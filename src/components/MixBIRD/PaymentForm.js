...
export default function PaymentForm() {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            try {
                const response = await axios.get(`http://birdsellingapi-001-site1.ctempurl.com/api/Product/GetProduct?category_id=${category}`);
            ...
            <Grid container spacing={3}>
                ...
                {selectedProduct && (
                    <Grid item xs={12}>
                        ...
                        <Typography>{`Sex: ${selectedProduct.sex ? 'Male' : 'Female'}`}</Typography>
                    </Grid>
                )}
                <Grid item xs={12}>
                    <FormControlLabel
